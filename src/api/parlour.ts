...
export const getParlours = query(
  async (paginateRequest: PaginateRequest): Promise<ParlourResponse> => {
    "use server";

    const query = toQueryParams(paginateRequest);
    const res = await fetchApi<PaginateResponse<Parlour>>(
      `/api/parlours?${query}`,
    );
...
export const getParlourById = query(
  async (id: number): Promise<Parlour | null> => {
    "use server";
...
export const deleteParlourById = action(
...
export const createParlour = action(
  async (body: ParlourRequest): Promise<Parlour> => {
    "use server";
...
export const updateParlour = action(
  async (id: number, body: ParlourRequest): Promise<Parlour> => {
    "use server";
