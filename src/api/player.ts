...
export const getPlayers = query(
  async (paginateRequest: PaginateRequest): Promise<PlayerResponse> => {
    ...
    const res = await fetchApi<PaginateResponse<Player>>(
      `/api/players?${query}`,
    );
...
export const createPlayer = action(
  async (body: PlayerRequest): Promise<Player> => {
    "use server";
...
export const updatePlayer = action(
  async (id: number, body: EditPlayerRequest): Promise<Player> => {
    "use server";
...
export const updatePasswordPlayer = action(
  async (body: ChangePasswordPlayer): Promise<boolean> => {
    "use server";

    const res = await fetchApi<ResponseData<boolean>>(
