...
export const getProvinces = query(
  async (paginateRequest: PaginateRequest): Promise<ProvinceResponse> => {
    "use server";
    const query = toQueryParams(paginateRequest);
    const res = await fetchApi<PaginateResponse<Province>>(
      `/api/provinces?${query}`,
    );
...
export const getProvinceById = query(
  async (id: number): Promise<Province | null> => {
    "use server";
