...
export const getMatches = query(
  async (paginateRequest: PaginateRequest): Promise<MatchResponse> => {
    "use server";

    const query = toQueryParams(paginateRequest);
    const res = await fetchApi<PaginateResponse<Match>>(
      `/api/matches?${query}`,
    );
...
export const updateMatch = action(
  async (id: number, body: MatchUpdate): Promise<Match> => {
    "use server";
...
export const updateMatchPoint = action(
  async (id: number, body: PointMatchUpdate): Promise<Match> => {
    "use server";

    const res = await fetchApi<ResponseData<Match>>(
