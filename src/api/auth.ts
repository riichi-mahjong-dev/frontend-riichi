...
export const loginUser = action(async (username: string, password: string, type: 'admin' | 'player'): Promise<string|undefined> => {
  "use server";

  const res = await fetchApi<ResponseData<AuthResponse>>(`/auth/login/${type}`, {
      method: 'POST',
