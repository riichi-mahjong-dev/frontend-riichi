...
export default function InputField({
  ...
  return (
    <div class="w-full">
      ...
      <div
        class={`flex w-full bg-white p-4 rounded border ${error && error() ? "dark:border-rose-800" : "dark:border-mj-green"} ${error && error() ? "border-rose-700" : "border-mj-green-400"}`}
      >
        <input
      ...
      <Show when={error && error()}>
        <span class="text-rose-700">{error && error()}</span>
      </Show>
    </div>
  );
}
