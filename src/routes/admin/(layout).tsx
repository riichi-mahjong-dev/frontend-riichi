...
const menus: Array<MenuType> = [
  ...
  },
  {
    ...
    path: [
      "^/admin/parlour$",
      "^/admin/parlour/create$",
      "^/admin/parlour/\\d+/edit",
      "^/admin/parlour/\\d+",
    ],
  },
  {
    ...
    <>
      ...
      <AdminProvider>
        <SideMenu
          ...
          menus={menus.filter(
            (val) =>
              user()?.user?.role === "super-admin" || val.label !== "Admin",
          )}
        ...
        >
          <div class="mt-header-mobile md:mt-header ml-0 md:ml-sidebar">
            <div
              class={`px-10 py-7-5 bg-default-background ${menuSelected() === 0 ? "bg-white md:bg-default-background" : ""}`}
            >
              {props.children}
            </div>
          </div>
