  ...
  return (
    <>
      <Show when={shouldShowBar()}>
        <div class="bg-white shadow z-50 h-20 w-full">
          <div class="flex items-center justify-between mx-auto h-full max-w-[930px] xl:px-0 px-8">
            <div class="text-black">Logo</div>
            <div onClick={() => {
              navigate("/login");
            }} class="flex py-2 px-12 text-black justify-center items-center h-12 text-xl font-bold bg-mj-green-300 text-white rounded">Login</div>
          </div>
        </div>
      </Show>
      {props.children}
